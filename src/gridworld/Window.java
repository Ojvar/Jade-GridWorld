...
class Statistics extends AbstractTableModel implements Observer {
	...
	public void setEnvView(EnvView view) {
		...
		// Changed SA: On a screen refresh of the environment update yourself (call the
	...
	public void update() {
		...
		// Changed SA: This is better, this will not change the layout of the complete
		// table!
...
public class Window extends JFrame {
	...
	public Window(final LogicalEnv env) {
		...
		clear.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				final int rv = JOptionPane.showConfirmDialog(Window.this,
						"Are you sure you want to clear the environment", "Confirm clear", JOptionPane.YES_NO_OPTION);
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (_lastFile == null) {
					JOptionPane.showMessageDialog(Window.this,
							...
							"Nothing to revert to", JOptionPane.ERROR_MESSAGE);
					return;
				}

				// popup confirmation dialog
				final int rv = JOptionPane.showConfirmDialog(Window.this,
						"Are you sure you want revert to " + _lastFile.getPath(), "Confirm revert to saved",
				...
				try {
					env.clear();
					final FileInputStream stream = new FileInputStream(_lastFile);
					env.load(stream);
				} catch (Exception ex) {
		...
		resize.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				// show dialog
				String size = (String) JOptionPane.showInputDialog(Window.this,
						"Resize environment (X * Y) (X, Y) (X x Y) (X Y)", "Resize environment",
						JOptionPane.PLAIN_MESSAGE, null, null, env.getWidth() + " * " + env.getHeight());
				...
				if ((size != null) && (size.length() > 0)) {
					...
					try {
						Dimension d = new Dimension(Integer.parseInt(x), Integer.parseInt(y));

						env.setSize(d);
					} catch (NumberFormatException e1) {
						JOptionPane.showMessageDialog(Window.this,
								"Invalid number format, it should be one of (X * Y) (X, Y) (X x Y) (X Y)",
								"Error parsing size", JOptionPane.ERROR_MESSAGE);
					}
				}
			}
		});
		...
		setid.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				// show dialog
				String objType = ((String) JOptionPane.showInputDialog(Window.this,
				...
				if (objType.length() == 0 || invalid) {
					JOptionPane.showMessageDialog(Window.this,
							...
							"Invalid object type", JOptionPane.ERROR_MESSAGE);
					return;
				}
		...
		senserange.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String range = (String) JOptionPane.showInputDialog(Window.this, "Set agent sensor range in cells",
		...
		about.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				JOptionPane.showMessageDialog(Window.this,
						...
						"About BlockWorld", JOptionPane.INFORMATION_MESSAGE);
			}
		});
		...
		final JSplitPane sp = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, envView, statsView);
